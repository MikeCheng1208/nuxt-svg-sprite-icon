...
- 🚀 **自動生成 SVG  Sprite** - 自動掃描並轉換 SVG 檔案為 Sprite 格式
...
export default defineNuxtConfig({
  ...
  svgSprite: {
    ...
    // 預設 Sprite 名稱（用於輸入目錄根目錄中的檔案）
    defaultSprite: 'icons',
    
    // 所有 svg-icon 預設 CSS 類別
...
| `output` | `string` | `'~/assets/sprite/gen'` | 生成 Sprite 檔案的目錄 |
| `defaultSprite` | `string` | `'icons'` | 預設 Sprite 名稱（用於根輸入目錄中的 SVG） |
...
| 屬性 | 類型 | 預設值 | 描述 |
|------|------|--------|------|
| `name` | `string` | - | **必要。** 圖示名稱（支援巢狀路徑如 `user/example`） |
...
2. **驗證建構**：確保模組在您的輸出目錄中生成了 Sprite 檔案
...
- 所有 Sprite 會在頁面載入時注入到 DOM 中
- 對於大型圖示集，考慮將圖示組織到不同的資料夾/ Sprite 中
